...
/** Handle for the periodic NDP injection timer. */
static esp_timer_handle_t s_ndp_timer = NULL;

/**
 * NDP injection interval in milliseconds.
 * 50ms = 20 Hz — provides consistent CSI measurements for sensing.
 */
#define NDP_INJECT_INTERVAL_MS  50
...
void csi_collector_init(void)
{
    ...
    /* Start periodic NDP injection for consistent CSI sensing. */
    csi_collector_start_ndp_timer();
}
...
/**
 * Timer callback for periodic NDP injection.
 */
...
void csi_collector_start_ndp_timer(void)
{
    if (s_ndp_timer != NULL) {
        ESP_LOGW(TAG, "NDP timer already running");
        return;
    }
    ...
    uint64_t period_us = (uint64_t)NDP_INJECT_INTERVAL_MS * 1000;
    err = esp_timer_start_periodic(s_ndp_timer, period_us);
    if (err != ESP_OK) {
    ...
    ESP_LOGI(TAG, "NDP injection active: every %d ms (%.0f Hz)",
             NDP_INJECT_INTERVAL_MS, 1000.0 / NDP_INJECT_INTERVAL_MS);
}
